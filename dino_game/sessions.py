...
class ReplayListSession:
    ...
    def run(self):
        rt.browse_replay_files(self.stdscr, rt.list_replay_files())


class CompetitionSession:
    def __init__(self, stdscr, renderer: rt.Renderer, cli_args: rt.CliArgs, replay_path: str):
        self.stdscr = stdscr
    ...
    def run(self):
        replay_player = rt.ReplayPlayer.from_file(self.replay_path)
        record_path = (
            self.cli_args.record_path
            or rt.default_replay_path("competitive", replay_player.seed)
        )
        competition = rt.CompetitionRun(
            replay_player,
            source_replay=self.replay_path,
            record_path=record_path,
        )
        rt.run_competition_loop(self.stdscr, self.renderer, competition)
...
class PlaySession:
    def __init__(
            ...
            cli_args: rt.CliArgs,
            replay_player: rt.ReplayPlayer | None = None):
        self.stdscr = stdscr
        self.renderer = renderer
        self.cli_args = cli_args
        ...
        self.run_index = 1
        self.manual_input = rt.ManualInputState()
        self.pause_state = rt.PauseState()
        self.event_frame = 0
    ...
    def _new_game(self):
        if self.replay_player:
            return rt.DinoGame(rng=random.Random(self.replay_player.seed)), None
        return rt.start_recording_run(
            self.mode,
            self.cli_args.record_path,
            self.run_index,
        )
    ...
    def _new_agent(self):
        ...
        if self.cli_args.mode == "llm":
            try:
                debug_path = (
                    rt.debug_log_path_for_replay(self.recorder.path)
                ...
                return rt.RuleAgent(), "Rule Agent (LLM config unavailable)"
        if self.cli_args.mode == "agent":
            return rt.RuleAgent(), "Rule Agent"
        return None, ""
        ...
        try:
            while True:
                ...
                if key == ord('q') or key == ord('Q'):
                    break
                now = time.monotonic()
                self.pause_state = rt.next_pause_state(self.pause_state, key, now)
                ...
                if self.pause_state.status != "running":
                    self.renderer.draw(
                        ...
                        self.pause_state,
                        now,
                        cached_frames_view=rt.cached_frames_view_for_agent(
                            self.agent,
                            self.event_frame + 1,
                        ),
                    )
    ...
    def _handle_game_over(self, key: int):
        ...
        if rt.should_reset_after_game_over(key, agent_active=bool(self.agent)):
            self.run_index += 1
            self.game, self.recorder = self._new_game()
            self.manual_input = rt.ManualInputState()
            self.pause_state = rt.PauseState()
            self.event_frame = 0
            if isinstance(self.agent, rt.LLMAgent):
    ...
    def _next_action(self, key: int) -> str | None:
        ...
        if self.replay_player:
            ...
            if not self.replay_player.has_frame(self.event_frame):
                ...
                return None
            action = self.replay_player.action_for_frame(self.event_frame)
            rt.apply_game_action(self.game, action)
            return action
        ...
        if isinstance(self.agent, rt.LLMAgent):
            ...
            if self.agent.needs_loading(next_frame):
                self.renderer.draw(
                    self.game,
                    self.agent_name,
                    loading_text=rt.LLM_LOADING_TEXT,
                    cached_frames_view=rt.cached_frames_view_for_agent(
                        self.agent,
                        next_frame,
                    ),
        ...
        action = rt.manual_action_from_key(self.manual_input, key)
        rt.apply_game_action(self.game, action)
        return action
    ...
    def _update_game(self, action: str):
        ...
        if self.game.game_over:
            rt.debug_log_llm_game_over(
                self.agent,
                self.game,
                frame=self.event_frame,
                action=action,
            )
            rt.finish_recording(self.recorder)
...
def session_for_cli_args(stdscr, cli_args: rt.CliArgs):
    if cli_args.command == "replay" and cli_args.replay_action == "list":
        return ReplayListSession(stdscr)

    renderer = rt.Renderer(stdscr)
    if cli_args.command == "compete":
        replay_path = cli_args.competition_path or rt.select_replay_file(
            stdscr,
            rt.list_replay_files(),
        )
        if not replay_path:
            return None
    ...
    if cli_args.command == "replay":
        replay_path = cli_args.replay_path or rt.select_replay_file(
            stdscr,
            rt.list_replay_files(),
        )
        if not replay_path:
        ...
        return ReplaySession(
            ...
            renderer,
            cli_args,
            rt.ReplayPlayer.from_file(replay_path),
        )
